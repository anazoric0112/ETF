...
public class TransakcijeResource {
    ...
    @Path("{u}/{pw}")
    @TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
    public Response plati(@PathParam("u") String u, @PathParam("pw") String pw) throws JMSException{
        JMSContext c=cf.createContext();
        JMSProducer producer=c.createProducer();
        JMSConsumer consumer=c.createConsumer(q);
        
        TextMessage tm=c.createTextMessage("0#"+u+"#"+pw);
        tm.setStringProperty("type","narudzbine");
        producer.send(t,tm);
        ...
        if (msg instanceof TextMessage){
            ...
            if (txt.startsWith("Pogresn") || txt.startsWith("Nemate"))
